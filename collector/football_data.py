...
Writes:
  docs/data/wire.json              latest batch, what the page shows
  docs/data/archive/YYYY-MM-DD.json   every batch, kept forever
  docs/data/mood.json              one aggregate score per day
  docs/data/rumours.json           per-subject tracking with temperature
  docs/data/usage.csv              appended by claude_client
"""
...
def update_mood(items, today):
    ...
    for it in items:
        ...
        den += w
    score = round((num / den) * 50, 1) if den else 0.0  # -100..100
...
def update_rumours(items, today):
    ...
    # A rumour nobody has repeated in three weeks is dead. Keep it; the graveyard
